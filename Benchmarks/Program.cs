...
namespace Benchmarks
{
    class Program
    {
        ...
        {
            int pointBatchSize = 1024;
            if (args.Length >= 1)
            {
                pointBatchSize = int.Parse(args[0]);
            }
            bool runDoubles = true;
            if (args.Length >= 2)
            {
                runDoubles = bool.Parse(args[1]);
            }
            ...
            var s = new IterationBenchmarks(pointBatchSize);
            ...
            using (s.SetContext("OpenCL GPU - doubles",
                openCLGuard: device => runDoubles && device.DoubleFloatingPointConfiguration != FloatingPointConfiguration.None))
            {
